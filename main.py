...
async def root():
    return JSONResponse(
        content={"message": "Welcome to the Student Marks API. Use /api?name=X&name=Y to get marks."},
        headers={"Access-Control-Allow-Origin": "*"}
    )

@app.get("/api", response_class=JSONResponse)
async def get_marks(request: Request, name: Optional[List[str]] = Query(default=None)):
    ...
    if name is None:
        return JSONResponse(content={"marks": []}, headers=headers)
    ...
    return JSONResponse(content={"marks": result}, headers=headers)

# Handle preflight OPTIONS requests for CORS
@app.options("/{path:path}", response_class=JSONResponse)
...
async def catch_all(request: Request, path: str):
    headers = {"Access-Control-Allow-Origin": "*"}
    return JSONResponse(
        content={"message": f"Path '{path}' not found. Use /api?name=X&name=Y to get marks."},
        headers=headers
    )
